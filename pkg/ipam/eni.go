...
// startMultiCloudDeviceConfigurator watches CiliumNode upsert events and
// calls configureMultiCloudInterfaces each time, so newly attached ENIs
// also get rp_filter=0 without requiring an agent restart.
func startMultiCloudDeviceConfigurator(
	logger *slog.Logger,
	jg job.Group,
	nodeResource agentK8s.LocalCiliumNodeResource,
	sysctlMgr sysctl.Sysctl,
) {
	jg.Add(
		job.Observer(
			"multicloud-device-configurator",
			func(ctx context.Context, ev resource.Event[*ciliumv2.CiliumNode]) error {
				defer ev.Done(nil)
				if ev.Kind != resource.Upsert {
					return nil
				}
				configureMultiCloudInterfaces(logger, sysctlMgr)
				return nil
			},
			nodeResource,
		),
...
// configureMultiCloudInterfaces disables rp_filter on all non-loopback interfaces.
// Multi-homed nodes with secondary ENIs will otherwise drop packets that arrive
// on a non-primary interface but whose source is reachable via the primary route.
func configureMultiCloudInterfaces(logger *slog.Logger, sysctlMgr sysctl.Sysctl) {
	links, err := netlink.LinkList()
	if err != nil {
		logger.Warn("multicloud: failed to list interfaces for rp_filter setup", logfields.Error, err)
		return
	}
	for _, link := range links {
